...
export default function HistActR() {

    const [ActRealizadas, setActRealizadas] = useState([]);
    ...
    useEffect(() => {
        async function fetchActividades () {
            ...
            if (user) {
                const actRef = db.collection('Usuarios').doc(user.uid).collection('ActividadR');
        ...
        <View style={styles.container}>
            ...
            <LinearGradient colors={['#93D5DD', '#32909C']} style={styles.table}>
                    <View style={styles.tableRow}>
                        ...
                        <Text style={styles.tableCellHeader}>Intesidad</Text>
                        
                    </View>
                    {ActRealizadas.map(actividades => (
                        <View key={actividades.id} style={styles.tableRow}>
                            <Text style={styles.tableCell}>{actividades.actividad}</Text>
                            <Text style={styles.tableCell}>{actividades.fecha}</Text>
                            <Text style={styles.tableCell}>{actividades.intensidad}</Text>
                        </View>
                    ))}
                </LinearGradient>
        </View>
