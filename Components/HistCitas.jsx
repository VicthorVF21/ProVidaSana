...
export default function HistCitas() {
    ...
    useEffect(() => {
        async function fetchCitas () {
            ...
            if (user) {
                const Ref = db.collection('Usuarios').doc(user.uid).collection('CitasM');
                const snapshot = await Ref.get();
                if (!snapshot.empty) {
                    const fetchedCitas = snapshot.docs.map(doc => ({
                        id: doc.id,
                        ...doc.data(),
                    }));
    ...
    return (
        <View style={styles.container}>
            ...
            <Ionicons style={styles.cardIcon1} name="document-text-outline" size={30} color="#000" />
               <Text style={styles.TextCard}>Registro de Citas</Text>
            </View>
            ...
            <LinearGradient colors={['#93D5DD', '#32909C']} style={styles.table}>
                    ...
                    {citas.map(cita => (
                        <View key={cita.id} style={styles.tableRow}>
                            ...
                            <Text style={styles.tableCell}>{cita.especialidad}</Text>
                            <Text style={styles.tableCell}>{cita.ubicacion}</Text>
                        </View>
                    ))}
                </LinearGradient>
        </ScrollView>
