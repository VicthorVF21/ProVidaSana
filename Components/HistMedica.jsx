...
export default function HistMedica() {
    ...
    useEffect(() => {
        async function fetchMed () {
            ...
            if (user) {
                const Ref = db.collection('Usuarios').doc(user.uid).collection('Medicamentos');
                const snapshot = await Ref.get();
                if (!snapshot.empty) {
                    const fetchedMed = snapshot.docs.map(doc => ({
                        id: doc.id,
                        ...doc.data(),
                    }));
                    setMedica(fetchedMed);
                }
            }
        };
        ...
        <View style={styles.container}>
            ...
            <View style={styles.ContainerButton}>
                ...
                <View style={styles.card}>
                    <Text style={styles.cardText}>Gestión de Medicamentos</Text>
            ...
            <Ionicons style={styles.cardIcon1} name="document-text-outline" size={30} color="#000" />
               <Text style={styles.TextCard}>Registro de Medicamentos</Text>
            </View>
            <ScrollView style={styles.tableContainer}>
            <LinearGradient colors={['#93D5DD', '#32909C']} style={styles.table}>
                    <View style={styles.tableRow}>
                        <Text style={styles.tableCellHeader}>Nombre</Text>
                        <Text style={styles.tableCellHeader}>Dosis</Text>
                        <Text style={styles.tableCellHeader}>Frecuencia diaria</Text>
                        <Text style={styles.tableCellHeader}>Cantidad por dia</Text>
                    </View>
                    {Medica.map(med => (
                        <View key={med.id} style={styles.tableRow}>
                            <Text style={styles.tableCell}>{med.Medicamentos}</Text>
                            <Text style={styles.tableCell}>{med.Dosis}</Text>
                            <Text style={styles.tableCell}>{med.Frecuencia}</Text>
                            <Text style={styles.tableCell}>{med.CantidadPdia}</Text>
                        </View>
                    ))}
                </LinearGradient>
        </ScrollView>
